...
def run(bot_key, tmp, config_path):
    ...
    application.bot.set_my_commands(BotCommand("start", "开始和基础的帮助"))
    application.bot.set_my_commands(BotCommand("set", "设置相关参数"))
    application.bot.set_my_commands(BotCommand("get", "获取相关参数"))
    application.bot.set_my_commands(BotCommand("run", "运行定时推送任务"))
    application.bot.set_my_commands(BotCommand("stop", "停止定时推送任务"))
    application.bot.set_my_commands(BotCommand("cookie_verify", "查询当前cookie可用性"))
    application.bot.set_my_commands(BotCommand("add_channel", "添加频道到推送列表"))
    application.bot.set_my_commands(BotCommand("del_channel", "从推送列表中删除特定频道"))
    application.bot.set_my_commands(BotCommand("del_all_channel", "清空推送列表"))
    application.bot.set_my_commands(BotCommand("status", "查看当然任务状态"))

    application.bot.set_my_description("一个推送pixiv账号关注画师更新的机器人,可以自动将更新的作品推送到频道中,使用/start开始")
    application.bot.set_my_short_description("一个推送pixiv账号关注画师更新的机器人,可以自动将更新的作品推送到频道中")
